...
argument-hint: "<template> [project-name]   templates: explainer | reel | product-demo | talking-head | faceless-story"
...
1. Resolve the template from `$ARGUMENTS` (default `explainer`). Valid: `explainer`, `reel`,
   `product-demo`, `talking-head`, `faceless-story`. If unknown, list the options and stop.
